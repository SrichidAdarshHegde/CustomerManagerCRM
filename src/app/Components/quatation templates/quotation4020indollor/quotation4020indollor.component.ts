...
@Component({
  selector: 'app-quotation4020indollor',
  templateUrl: './quotation4020indollor.component.html',
  styleUrls: ['./quotation4020indollor.component.css']
})
export class Quotation4020indollorComponent {
  ...
  selectedcustID: any;
  customerdata: any;
  TemplateName: string = "Quotation 4020 In Dollar";
  BasicSystemQty: string = '1 Set';
  BasicSystemPrice: number =14715    ; 
  ...
  OptionalPriceF: number = 3500; 

  OptionalQtyG:number=1;
  OptionalPriceG: number = 4200; 

  OptionalQtyH:number=1;
  OptionalPriceH: number = 13490; 
  
  //editable

  editableQty: string = '1 Set';
  editablePrice:number=14715;
  ...
  editablePriceC:number=190;

  editableQtyD:number=1;
  editablePriceD:number=4750;

  editableQtyE:number=1;
  editablePriceE:number=3500;
...
getBillingAddress(){
  ...
  fetchTemplate() {
    this.regSv.gettemplatedetails4020INdollor(this.RefID).subscribe((response: any) => {
  ...
  KindAttention:this.KindAttention,
    ...
    this.regSv.postSavequotationtemplate4020INDOLLOR(templateData).subscribe((response:any )=>
    {
      if(response !=null){
